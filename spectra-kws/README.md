...
```
INMP441 ─I²S/DMA─► 1s ring buffer (32KB) ─► MFCC front-end ─► DS-CNN (INT8) ─► posterior smoothing
                                                                                │
                                                                     "Spectra" detected?  yes
                                                                                │
     flush last 500ms ─► open WebSocket over 5 GHz Wi-Fi ─► stream live audio ─► cloud ASR ─► transcript
```

| Stage | Runs on | Latency budget |
|---|---|---|
| I²S DMA capture → 1s ring buffer | ESP32-C5 hardware | 0 µs CPU |
| MFCC extraction (49×10 matrix) | ESP32-C5 CPU | < 5 ms / frame |
| DS-CNN inference (INT8) | ESP32-C5 CPU (TFLite Micro) | < 20 ms |
| Trigger logic (posterior smoothing) | ESP32-C5 CPU | < 1 ms |
| Flush 500ms ring buffer + WS open | ESP32-C5 + Wi-Fi 6 | ~50 ms |
| Cloud ASR first token | Server (Whisper-class) | 300–500 ms |
| **Total end-to-end** | | **< 800 ms** |

> **Ring buffer note**: The ring buffer is 1 second (not 500ms) because MFCC needs a full 1-second window.
> On trigger, only the most recent 500ms is flushed to the ASR server — the pre-trigger audio
> that ensures the first syllable after "Spectra" is never lost.

...
static int16_t s_flush_buf[SPECTRA_FLUSH_SAMPLES];  /* 500ms pre-trigger audio */
...
static void spectra_task(void *arg)
{
    ...
    while (1) {
        ...
        if (triggered) {
            ...
            /* Flush last 500ms of ring buffer (pre-trigger audio) */
            size_t flushed = i2s_capture_read(s_flush_buf, SPECTRA_FLUSH_SAMPLES);
            ...
            ws_stream_audio(s_flush_buf, flushed);
